...
\hline
\texttt{Ctrl+S} & Quick Save & Saves current layout with optional tag from input field \\
\hline
\texttt{Ctrl+Q} & Exit Application & Closes program completely, respects Auto-Save setting \\
\hline
\end{tabular}
...
\begin{enumerate}
    ...
    \item If error persists, check Windows Explorer is running (Ctrl+Shift+Esc, look for explorer.exe)
\end{enumerate}
...
Currently, the program does not support command-line arguments. Future versions may include:
\begin{itemize}
    \item Silent backup mode: \texttt{--backup --silent}
    \item Silent restore mode: \texttt{--restore latest --silent}
    \item Specify backup file: \texttt{--restore "filename.json"}
\end{itemize}
