...
The application can be operated via terminal for automation and
scripting without using the graphical interface.

\begin{description}[style=nextline]
  \item[\texttt{--save}] Performs an immediate backup with the tag
    "CLI Quick Save" and exits.
  \item[\texttt{--restore <filename>}] Restores icons from the
    specified JSON file.
  \item[\texttt{--latest}] Combined with \texttt{--restore}, it
    automatically selects the most recent backup.
  \item[\texttt{--silent}] Runs the operation in the background
    without showing any windows or taskbar icons.
  \item[\texttt{--enable-scaling}] Forces coordinate scaling if a
    resolution mismatch is detected during CLI restore.
\end{description}
